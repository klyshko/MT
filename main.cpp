...
void read_PDB(const char* filename_xyz, const char* filename_ang){
    ...
    UpdateLJPairs();
#endif
    printf("done building topology.\n");
}

void UpdateLJPairs()
{
    memset(top.LJCount, 0, par.Ntot*par.Ntr*sizeof(int));
    for(int tr = 0; tr < par.Ntr; tr++)
    {
        for(int i = 0; i < par.Ntot; i++)
        {
            ...
            {
                float dr = sqrt(pow(r[i+tr*par.Ntot].x-r[j+tr*par.Ntot].x,2)+
                                pow(r[i+tr*par.Ntot].y-r[j+tr*par.Ntot].y,2)+
                                pow(r[i+tr*par.Ntot].z-r[j+tr*par.Ntot].z,2));

                if((dr < par.ljpairscutoff) && (i != j))
                {
                    top.LJCount[i + tr * par.Ntot]++;
                    if(top.LJCount[i + tr * par.Ntot] > top.maxLJPerMonomer)
                    {
                        top.maxLJPerMonomer = top.LJCount[i + tr * par.Ntot];
                        int* tmp = NULL;
                        tmp = (int*)realloc(top.LJ, sizeof(int)*par.Ntot*par.Ntr*top.maxLJPerMonomer);
                        if(tmp!=NULL)
                        {
                            top.LJ = tmp;
                        }
                        ...
                        {
                            printf("Unable to reallocate memory during LJ pairs update\n");
                            exit(0);
                        }
                    }
                    if( ((top.LJCount[i+tr*par.Ntot]-1)*par.Ntot*par.Ntr+par.Ntot*tr+i >= par.Ntot*par.Ntr*top.maxLJPerMonomer) ||
                        ((top.LJCount[i+tr*par.Ntot]-1)*par.Ntot*par.Ntr+par.Ntot*tr+i < 0))
                    {
                        printf("KAPUT %d %d %d\n", i, tr, top.LJCount[i]);
                        exit(0);
                    }
                    top.LJ[(top.LJCount[i+tr*par.Ntot]-1)*par.Ntot*par.Ntr+par.Ntot*tr+i] = j+par.Ntot*tr;
                }
            }
        }
    }
...
void OutputEn(Energies en)
{
    printf("ENERGIES harm = %f long = %f lat = %f psi = %f fi = %f theta = %f lj = %f\n", en.U_harm, en.U_long, en.U_lat, en.U_psi, en.U_fi, en.U_teta, en.U_lj);
}
...
{   
    ...
    for(int i = 0; i < par.Ntot; i++)
    {
        ...
        top.lateral[2*i] = LARGENUMBER;
        top.lateral[2*i + 1] = LARGENUMBER;
    }
    ...
    for(int i = 0; i < par.Ntot; i++)
    {
        ...
        float curMinDist = PAIR_CUTOFF;

        for(int j = 0; j < par.Ntot; j++)
        {
            if(top.harmonic[i]*top.harmonic[j] <= 0)
            {
                rj = r[j];
                cos_fij = cosf(rj.fi);
                sin_fij = sinf(rj.fi);
                ...
                if(dr < curMinDist)
                {
                    curMinDist = dr;
                    if(top.harmonic[i] < 0)
                        top.longitudinal[i] = j;
                    else
                        top.longitudinal[i] = -j;

                    top.longitudinalCount[i] = 1;
                }
            }
        }
    }
    ...
    for(int i = 0; i < par.Ntot; i++){
        ...
        float curMinDist[2] = {PAIR_CUTOFF, PAIR_CUTOFF};
        int latFlag[2] = {0, 0};
            ...
            if (i != j) {
                ...
                {   
                    ...
                    if (ind == 0) {
                        ...
                        if ((dr < curMinDist[1]) && (top.lateral[2 * i + 0] + j != 0)) {
                            curMinDist[1] = dr;
                            top.lateral[2 * i + 1] = j;
                            latFlag[1] = 1; //
                        }
        ...
        //top.lateralCount[i] = latFlag[0] + latFlag[1];
    }
